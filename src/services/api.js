...
// Envia todos os dados do censo (7 etapas) para o banco
export async function enviarCenso(formData) {
  const id_responsavel = sessionStorage.getItem("id_responsavel");

  // 1. Cria a pessoa com TEA + perfil clínico + perfil censo completo
  ...
  if (formData.profissionais && formData.profissionais.trim() !== "") {
    const prof = await req("/profissional", {
      method: "POST",
      body: JSON.stringify({
        nome_profissional: formData.profissionais,
