...
# ── Classe Responsavel ────────────────────────────────────────
class Responsavel:
    def criar(self, email, senha, data_aceite):
        with db.conectar() as conn:
            try:
                cur = conn.execute(
                    "INSERT INTO RESPONSAVEL (Email, Senha, Data_Aceite) VALUES (?, ?, ?)",
                    (email, senha, data_aceite),
                )
                conn.commit()
    ...
    def listar(self):
        with db.conectar() as conn:
            cur = conn.execute("SELECT ID_Responsavel, Email, Data_Aceite FROM RESPONSAVEL")
...
def criar_responsavel():
    d = request.get_json()
    dados, status = responsavel.criar(d["email"], d["senha"], d["data_aceite"])
...
def dados_dashboard():
    with db.conectar() as conn:
        responsaveis = db.linhas(conn.execute(
            "SELECT ID_Responsavel, Email, Data_Aceite FROM RESPONSAVEL"
        ))
